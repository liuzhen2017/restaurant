...
public class PayApiController extends BaseController{
	...
	@ResponseBody
	public AjaxResult payment(String email, String tokenId, Double ammount,String code) {
		log.info("購買VIP開始");;
		...
		if(StringUtils.isNoneEmpty(code)) {
			...
			SysConfig selectByKey = configService.selectByKey(HttpConstants.VIPPrice);
		...
		try {
			...
			if(charge.getStatus().equals("succeeded")){
				...
				try {
					...
					Members membersById = membersService.selectMembersById(getUserInfo().getId());
					data.put("webToken", JsonWebTokenUtil.sign(membersById));
	...
	@ResponseBody
	public AjaxResult upgradeVIP(String email, String tokenId, Double ammount,String code) {
